...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const claim = await env.LEADS_DB.prepare(
    `SELECT id, listing_slug, claimed_at, data_json FROM claimed_listings
     WHERE provider_id = ? AND listing_slug = ?`
  ).bind(provider.id, slug).first<{ id: number; listing_slug: string; claimed_at: string; data_json: string }>();
...
export const onRequestPut: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const claim = await env.LEADS_DB.prepare(
    `SELECT id, data_json FROM claimed_listings WHERE provider_id = ? AND listing_slug = ?`
  ).bind(provider.id, slug).first<{ id: number; data_json: string }>();
